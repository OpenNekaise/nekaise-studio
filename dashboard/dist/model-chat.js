import { escapeHTML as e, time } from "./lib.js?v=615dc8e42cb2";
...
export function chatView(state) {
  return `<section class="model-page" aria-labelledby="model-title">
    <div class="section-heading"><div><div class="eyebrow">Latest student</div><h1 id="model-title">Model</h1>
    <p class="quiet">Talk to the latest completed model. Each message uses the newest available version.</p></div>
    ...
    <div class="model-messages" id="model-messages" aria-live="polite" aria-label="Conversation">${state.messages.length ? state.messages.map(m => `<article class="model-message ${m.role}"><div class="model-message-heading">${m.role === "user" ? "You" : "Model"}${m.model ? ` <span class="quiet">· Iteration ${e(m.model.round_number)} · ${e(m.model.id.slice(0, 8))}</span>` : ""}</div><div class="model-message-text">${e(m.content)}</div>${m.partial && m.role === "assistant" ? '<small class="quiet">Incomplete reply · excluded from conversation context</small>' : ""}</article>`).join("") : '<div class="model-empty">Ask a question, explore an idea, or try a follow-up.</div>'}</div>

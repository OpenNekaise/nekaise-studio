...
across **≥2 independent experiments**. Tag the supporting decision when adjudicating:

    python -m studio.cli decide <run_id> --metric <metric> --noise-band <band> \
      --finding <slug> --hypothesis "<short statement>"

Then run

    python -m studio.tools.crystallize_gate --finding <slug>

Exit 1 = do NOT write the skill — the finding stays an unvalidated run hypothesis until
...
- **Unvalidated ideas** → they stay in run decision metadata as hypotheses, not in a skill.
...
- New emergent skills go in the active workspace's
  **`skills/local/<name>.md`** (`$NEKAISE_WORKSPACE/skills/local/` for an external
  workspace; the repo path is the legacy fallback). They are user-owned and never
  pushed implicitly; everyone's local skills differ.
...
At the end of every `coapt-round` cycle, ask: *"what reusable, validated lesson did this

import { escapeHTML as e, number, dateLabel } from "./lib.js?v=615dc8e42cb2";
import { scoreTrend, trendLegend } from "./score-trend.js?v=615dc8e42cb2";
...
export function scoreChart(points, metric = "score", options = {}) {
  ...
  if (!valid.length) return `<div class="token-chart-empty">${options.emptyText ? e(options.emptyText) : metric === "score" || !metric ? "No completed independent evaluation yet." : `No ${e(m.label.toLowerCase())} measurements in the displayed observations.`}</div>`;
...
export function benchmarkCard(data, browser = {}) {
  ...
  return `<section class="panel usage-card eval-usage"><div class="usage-heading"><h2>Independent eval</h2><span class="usage-live"><i></i>${e(label)}</span></div><strong class="usage-total">${pct(p?.score)}</strong><p class="usage-description">Building energy${p ? ` · ${count(p.correct)} / ${count(p.n)} correct · ${e(p.release_name)}` : " · awaiting verified results"}</p><div class="token-chart">${scoreChart(points, "score", { baseline: baselinePoint })}</div><div class="usage-footer"><span>${gainText}</span><span>${p ? `${p.round_number ? `Scored iteration ${number(p.round_number)}` : "Scored starting model"} · ${e(dateLabel(p.evaluated_at))}` : "Training continues while evaluation runs"}</span>${p && data?.generated_at ? `<span>${freshness(data, p)}</span>` : ""}${data?.stale ? `<span>Earlier weights · ${number(data.lag_tokens)} retained-weight tokens behind</span>` : ""}${invalid ? `<span>${invalid} failed/incompatible observation(s) among recent issues</span>` : ""}${historyLine}</div>${details}</section>`;
}
...
export function benchmarkHistory(data, browser = {}) {
  ...
  const chart = scoreChart(shown, metricName, { baseline: baselinePoint, interactive: true, selected: selectedId, emptyText: showingOverview && !overview.length ? "No completed independent evaluation yet." : undefined });

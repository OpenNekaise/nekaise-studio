...
Two modes, unchanged since the decoupling reform:
  ...
  python tools/eval_bench.py --run-id <run_id> --split dev
  python tools/eval_bench.py --checkpoint openbmb/MiniCPM5-1B-Base --split dev
...
def eval_checkpoint(args) -> dict:
    ...
    out_dir = ACTIVE_WORKSPACE.scratch_dir / "bench_eval"
...
def main() -> int:
    ...
    who.add_argument("--run-id", help="immutable training run checkpoint")
    ...
    store = RunStore(REPO)
    if args.run_id:
        args.checkpoint = str(store.resolve_checkpoint(args.run_id))
    ...
    if args.split == "test" and args.checkpoint:
        ...
        audit = ACTIVE_WORKSPACE.scratch_dir / "bench_eval" / "test_split_audit.log"
    ...
    if args.run_id:
        store.record_metric(
            args.run_id, name="nekaise_bench", value=result["overall"], n=result["n"],
            split=result["split"], split_hash=result["bench_version"],
            evaluator="nekaise_bench", evaluator_version=result["bench_version"],
            diagnostic=True, metadata={"by_track": result.get("by_track"),
                                       "mode": result["mode"]},
        )
    ...
    print("EVAL_RESULT " + json.dumps({"run_id": args.run_id, **result},
